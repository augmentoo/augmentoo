...
from augmentoo.augmentations.spatial.rotate import keypoint_rotate
from augmentoo.augmentations.spatial.rotate90 import keypoint_rot90
from augmentoo.augmentations.spatial.shift_scale_rotate import keypoint_shift_scale_rotate
from augmentoo.core.decorators import angle_to_2pi_range
from augmentoo.targets import convert_keypoints_from_albumentations
from augmentoo.targets.keypoint import convert_keypoint_from_albumentations, convert_keypoint_to_albumentations, \
...
def test_compose_with_keypoint_noop(keypoints, keypoint_format, labels):
    ...
    if labels is not None:
        aug = A2.Compose([A2.NoOp(p=1.0)], keypoint_params={"format": keypoint_format, "label_fields": ["labels"]})
        transformed = aug(image=image, keypoints=keypoints, labels=labels)
    else:
        aug = A2.Compose([A2.NoOp(p=1.0)], keypoint_params={"format": keypoint_format})
...
def test_compose_with_keypoint_noop_error_label_fields(keypoints, keypoint_format):
    image = np.ones((100, 100, 3))
    aug = A2.Compose([A2.NoOp(p=1.0)], keypoint_params={"format": keypoint_format, "label_fields": "class_id"})
...
def test_compose_with_keypoint_noop_label_outside(keypoints, keypoint_format, labels):
    image = np.ones((100, 100, 3))
    aug = A2.Compose([A2.NoOp(p=1.0)], keypoint_params={"format": keypoint_format, "label_fields": list(labels.keys())})
...
def test_random_sized_crop_size():
    image = np.ones((100, 100, 3))
    keypoints = [(0.2, 0.3, 0.6, 0.8), (0.3, 0.4, 0.7, 0.9, 99)]
    aug = augmentoo.augmentations.crops.random_sized_crop.RandomSizedCrop(min_max_height=(70, 90), height=50, width=50, p=1.0)
    transformed = aug(image=image, keypoints=keypoints)
...
def test_random_resized_crop_size():
    image = np.ones((100, 100, 3))
    keypoints = [(0.2, 0.3, 0.6, 0.8), (0.3, 0.4, 0.7, 0.9, 99)]
    aug = augmentoo.augmentations.crops.random_resized_crop.RandomResizedCrop(height=50, width=50, p=1.0)
    transformed = aug(image=image, keypoints=keypoints)
    assert transformed["image"].shape == (50, 50, 3)
    assert len(keypoints) == len(transformed["keypoints"])
...
@pytest.mark.parametrize(
    ...
    [
        [A2.HorizontalFlip, [[0, 0]], [[2, 0]]],
        [A2.HorizontalFlip, [[2, 0]], [[0, 0]]],
        [A2.HorizontalFlip, [[0, 2]], [[2, 2]]],
        [A2.HorizontalFlip, [[2, 2]], [[0, 2]]],
        #
        [A2.VerticalFlip, [[0, 0]], [[0, 2]]],
        [A2.VerticalFlip, [[2, 0]], [[2, 2]]],
        [A2.VerticalFlip, [[0, 2]], [[0, 0]]],
        [A2.VerticalFlip, [[2, 2]], [[2, 0]]],
        #
        [A2.HorizontalFlip, [[1, 1]], [[1, 1]]],
        [A2.VerticalFlip, [[1, 1]], [[1, 1]]],
    ],
)
def test_keypoint_flips_transform_3x3(aug, keypoints, expected):
    transform = A2.Compose([aug(p=1)], keypoint_params={"format": "xy"})
...
@pytest.mark.parametrize(
    ["aug", "keypoints", "expected"],
    [
        [A2.HorizontalFlip, [[20, 30, 0, 0]], [[79, 30, 180, 0]]],
        [A2.HorizontalFlip, [[20, 30, 45, 0]], [[79, 30, 135, 0]]],
        [A2.HorizontalFlip, [[20, 30, 90, 0]], [[79, 30, 90, 0]]],
        #
        [A2.VerticalFlip, [[20, 30, 0, 0]], [[20, 69, 0, 0]]],
        [A2.VerticalFlip, [[20, 30, 45, 0]], [[20, 69, 315, 0]]],
        [A2.VerticalFlip, [[20, 30, 90, 0]], [[20, 69, 270, 0]]],
    ],
)
...
def test_keypoint_rotate90(keypoint, expected, factor):
    actual = keypoint_rot90(keypoint, factor, rows=100, cols=200)
...
def test_keypoint_rotate(keypoint, expected, angle):
    actual = keypoint_rotate(keypoint, angle, rows=100, cols=100)
...
def test_keypoint_scale(keypoint, expected, scale):
    actual = keypoint_scale(keypoint, scale, scale)
    np.testing.assert_allclose(actual, expected, atol=1e-7)
...
def test_keypoint_shift_scale_rotate(keypoint, expected, angle, scale, dx, dy):
    actual = keypoint_shift_scale_rotate(keypoint, angle, scale, dx, dy, rows=100, cols=200)
...
def test_compose_with_additional_targets():
    ...
    kp1 = [(15, 15), (55, 55)]
    aug = A2.Compose([augmentoo.augmentations.crops.center_crop.CenterCrop(50, 50)], keypoint_params={"format": "xy"}, additional_targets={"kp1": "keypoints"})
...
def test_coarse_dropout():
    aug = A2.Compose(
        [A2.CoarseDropout(min_holes=1, max_holes=1, min_height=128, max_width=128, min_width=128, max_height=128, p=1)],
        keypoint_params=A2.KeypointParams(format="xy"),
    )
