...
@pytest.mark.parametrize(
    ...
    get_image_only_transforms(
        ...
        except_augmentations={A2.FromFloat, A2.Normalize, A2.ToFloat},
    ),
)
...
@pytest.mark.parametrize(
    ...
    get_transforms(
        ...
        except_augmentations={
            ...
            augmentoo.augmentations.crops.random_crop_near_bbox.RandomCropNearBBox,
            augmentoo.augmentations.crops.random_sized_bbox_safe_crop.RandomSizedBBoxSafeCrop,
            augmentoo.augmentations.crops.center_crop.CenterCrop,
            augmentoo.augmentations.crops.crop.Crop,
            augmentoo.augmentations.crops.crop_non_empty_mask_if_exists.CropNonEmptyMaskIfExists,
            augmentoo.augmentations.crops.random_crop.RandomCrop,
            augmentoo.augmentations.crops.random_resized_crop.RandomResizedCrop,
            augmentoo.augmentations.crops.random_sized_crop.RandomSizedCrop,
...
@pytest.mark.parametrize(["augmentation_cls", "params"], [[
                                                              augmentoo.augmentations.crops.random_crop_near_bbox.RandomCropNearBBox, {"max_part_shift": 0.15}]])
...
@pytest.mark.parametrize(
    ...
    [
        ...
        [A2.Rotate, {"border_mode": cv2.BORDER_CONSTANT, "value": 100, "mask_value": 1}],
        [A2.SafeRotate, {"border_mode": cv2.BORDER_CONSTANT, "value": 100, "mask_value": 1}],
        [A2.ShiftScaleRotate, {"border_mode": cv2.BORDER_CONSTANT, "value": 100, "mask_value": 1}],
        [A2.OpticalDistortion, {"border_mode": cv2.BORDER_CONSTANT, "value": 100, "mask_value": 1}],
        [A2.ElasticTransform, {"border_mode": cv2.BORDER_CONSTANT, "value": 100, "mask_value": 1}],
        [A2.GridDistortion, {"border_mode": cv2.BORDER_CONSTANT, "value": 100, "mask_value": 1}],
        [A2.Affine, {"mode": cv2.BORDER_CONSTANT, "cval_mask": 1, "cval": 100}],
        [A2.PiecewiseAffine, {"mode": "constant", "cval_mask": 1, "cval": 100}],
    ],
)
...
@pytest.mark.parametrize(
    ...
    get_transforms(
        ...
        except_augmentations={
            ...
            A2.RGBShift,
            augmentoo.augmentations.crops.random_crop_near_bbox.RandomCropNearBBox,
            augmentoo.augmentations.random_fog.RandomFog,
            A2.RandomRain,
            A2.RandomShadow,
            augmentoo.augmentations.crops.random_sized_bbox_safe_crop.RandomSizedBBoxSafeCrop,
            A2.RandomSnow,
...
@pytest.mark.parametrize(
    ...
    [
        [A2.PadIfNeeded, {"min_height": 514, "min_width": 516}, (300, 200)],
        [A2.PadIfNeeded, {"min_height": 514, "min_width": 516}, (512, 516)],
        [A2.PadIfNeeded, {"min_height": 514, "min_width": 516}, (600, 600)],
        [
            A2.PadIfNeeded,
        ...
        [A2.PadIfNeeded, {"min_height": 512, "min_width": None, "pad_width_divisor": 128}, (300, 200)],
        [A2.PadIfNeeded, {"min_height": None, "min_width": 512, "pad_height_divisor": 128}, (300, 200)],
    ],
)
def test_pad_if_needed(augmentation_cls: Type[A2.PadIfNeeded], params: Dict, image_shape: Tuple[int, int]):
...
def test_perspective_order_points(points):
    ...
    res = A2.Perspective._order_points(points)
...
def test_perspective_valid_keypoints_after_transform(seed: int, scale: float, h: int, w: int):
    ...
    transform = A2.Compose(
        [A2.Perspective(scale=(scale, scale), p=1)], keypoint_params={"format": "xy", "remove_invisible": False}
    )
...
def test_pixel_domain_adaptation(kind):
    ...
    for img, ref_img in ((img_uint8, ref_img_uint8), (img_float, ref_img_float)):
        adapter = A2.PixelDistributionAdaptation(
...
def test_non_contiguous_input(augmentation_cls, params, bboxes):
    ...
    if augmentation_cls == augmentoo.augmentations.crops.random_crop_near_bbox.RandomCropNearBBox:
        # requires "cropping_bbox" arg
        aug = augmentation_cls(p=1, **params)
        aug(image=image, mask=mask, cropping_bbox=bboxes[0])
    elif augmentation_cls == augmentoo.augmentations.crops.random_sized_bbox_safe_crop.RandomSizedBBoxSafeCrop:
        # requires "bboxes" arg
        aug = A2.Compose([augmentation_cls(p=1, **params)], bbox_params=A2.BboxParams(format="pascal_voc"))
        aug(image=image, mask=mask, bboxes=bboxes)
    else:
        # standard args: image and mask
        if augmentation_cls == A2.FromFloat:
            # requires float image
            image = (image / 255).astype(np.float32)
            assert not image.flags["C_CONTIGUOUS"]
        elif augmentation_cls == A2.MaskDropout:
