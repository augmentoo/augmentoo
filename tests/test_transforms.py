...
def test_transpose_both_image_and_mask():
    image = np.ones((8, 6, 3))
    mask = np.ones((8, 6))
    augmentation = augmentoo.augmentations.spatial.transpose.Transpose(p=1)
...
def test_safe_rotate_interpolation(interpolation):
    ...
    aug = A2.SafeRotate(limit=(45, 45), interpolation=interpolation, p=1)
...
def test_rotate_interpolation(interpolation):
    image = np.random.randint(low=0, high=256, size=(100, 100, 3), dtype=np.uint8)
    mask = np.random.randint(low=0, high=2, size=(100, 100), dtype=np.uint8)
    aug = A2.Rotate(limit=(45, 45), interpolation=interpolation, p=1)
    data = aug(image=image, mask=mask)
...
def test_optical_distortion_interpolation(interpolation):
    image = np.random.randint(low=0, high=256, size=(100, 100, 3), dtype=np.uint8)
    mask = np.random.randint(low=0, high=2, size=(100, 100), dtype=np.uint8)
    aug = A2.OpticalDistortion(distort_limit=(0.05, 0.05), shift_limit=(0, 0), interpolation=interpolation, p=1)
    data = aug(image=image, mask=mask)
...
def test_grid_distortion_interpolation(interpolation):
    image = np.random.randint(low=0, high=256, size=(100, 100, 3), dtype=np.uint8)
    mask = np.random.randint(low=0, high=2, size=(100, 100), dtype=np.uint8)
    aug = A2.GridDistortion(num_steps=1, distort_limit=(0.3, 0.3), interpolation=interpolation, p=1)
    data = aug(image=image, mask=mask)
...
def test_grid_distortion_steps(size):
    ...
    aug = A2.GridDistortion(num_steps=size - 2, p=1)
...
def test_elastic_transform_interpolation(monkeypatch, interpolation):
    ...
    mask = np.random.randint(low=0, high=2, size=(100, 100), dtype=np.uint8)
    monkeypatch.setattr(
        "augmentoo.augmentations.geometric.ElasticTransform.get_params", lambda *_: {"random_state": 1111}
    )
    aug = A2.ElasticTransform(alpha=1, sigma=50, alpha_affine=50, interpolation=interpolation, p=1)
    data = aug(image=image, mask=mask)
...
@pytest.mark.parametrize(
    ...
    get_dual_transforms(
        custom_arguments={
            ...
            A2.PixelDropout: {"dropout_prob": 0.5, "mask_drop_value": 10, "drop_value": 20},
        },
        except_augmentations={augmentoo.augmentations.crops.random_crop_near_bbox.RandomCropNearBBox,
                              augmentoo.augmentations.crops.random_sized_bbox_safe_crop.RandomSizedBBoxSafeCrop, A2.PixelDropout},
    ),
)
...
@pytest.mark.parametrize(
    ...
    get_dual_transforms(
        ...
        except_augmentations={augmentoo.augmentations.crops.random_crop_near_bbox.RandomCropNearBBox,
                              ...
                              augmentoo.augmentations.crops.crop_and_pad.CropAndPad, A2.PixelDropout},
    ),
)
    ...
    aug = A2.Compose(
        [
            A2.OneOrOther(
                A2.Compose(
                    [
                        augmentoo.augmentations.crops.random_sized_crop.RandomSizedCrop(min_max_height=(256, 1025), height=512, width=512, p=1),
                        A2.OneOf(
                            [
...
def test_channel_droput():
    ...
    aug = A2.ChannelDropout(channel_drop_range=(1, 1), always_apply=True)  # Drop one channel

    transformed = aug(image=img)["image"]

    assert sum(transformed[:, :, c].max() for c in range(img.shape[2])) == 2

    aug = A2.ChannelDropout(channel_drop_range=(2, 2), always_apply=True)  # Drop two channels
    transformed = aug(image=img)["image"]
...
def test_equalize():
    ...
    aug = A2.Equalize(mask=mask_func, mask_params=["test"], p=1)
...
def test_crop_non_empty_mask():
    ...
    mask_1[0, 0] = 1
    crop_1 = np.array([[1]])
    aug_1 = augmentoo.augmentations.crops.crop_non_empty_mask_if_exists.CropNonEmptyMaskIfExists(1, 1)

    # test empty mask
    mask_2 = np.zeros([10, 10])
    crop_2 = np.array([[0]])
    aug_2 = augmentoo.augmentations.crops.crop_non_empty_mask_if_exists.CropNonEmptyMaskIfExists(1, 1)

    # test ignore values
    mask_3 = np.ones([2, 2])
    mask_3[0, 0] = 2
    crop_3 = np.array([[2]])
    aug_3 = augmentoo.augmentations.crops.crop_non_empty_mask_if_exists.CropNonEmptyMaskIfExists(1, 1, ignore_values=[1])
    ...
    mask_4[1, 1, 1] = 2
    crop_4 = np.array([[[1, 0]]])
    aug_4 = augmentoo.augmentations.crops.crop_non_empty_mask_if_exists.CropNonEmptyMaskIfExists(1, 1, ignore_channels=[1])

    # test full size crop
    mask_5 = np.random.random([10, 10, 3])
    crop_5 = mask_5
    aug_5 = augmentoo.augmentations.crops.crop_non_empty_mask_if_exists.CropNonEmptyMaskIfExists(10, 10)

    mask_6 = np.zeros([10, 10, 3])
    mask_6[0, 0, 0] = 0
    crop_6 = mask_6
    aug_6 = augmentoo.augmentations.crops.crop_non_empty_mask_if_exists.CropNonEmptyMaskIfExists(10, 10, ignore_values=[1])
...
def test_downscale(interpolation):
    ...
    aug = A2.Downscale(scale_min=0.5, scale_max=0.5, interpolation=interpolation, always_apply=True)
...
def test_crop_keypoints():
    ...
    aug = augmentoo.augmentations.crops.crop.Crop(0, 0, 80, 80, p=1)
    result = aug(image=image, keypoints=keypoints)
    assert result["keypoints"] == keypoints

    aug = augmentoo.augmentations.crops.crop.Crop(50, 50, 100, 100, p=1)
    result = aug(image=image, keypoints=keypoints)
...
def test_longest_max_size_keypoints():
    ...
    aug = A2.LongestMaxSize(max_size=100, p=1)
    result = aug(image=img, keypoints=keypoints)
    assert result["keypoints"] == [(18, 10, 0, 0)]

    aug = A2.LongestMaxSize(max_size=5, p=1)
    result = aug(image=img, keypoints=keypoints)
    assert result["keypoints"] == [(0.9, 0.5, 0, 0)]

    aug = A2.LongestMaxSize(max_size=50, p=1)
    result = aug(image=img, keypoints=keypoints)
...
def test_smallest_max_size_keypoints():
    ...
    aug = A2.SmallestMaxSize(max_size=100, p=1)
    result = aug(image=img, keypoints=keypoints)
    assert result["keypoints"] == [(90, 50, 0, 0)]

    aug = A2.SmallestMaxSize(max_size=5, p=1)
    result = aug(image=img, keypoints=keypoints)
    assert result["keypoints"] == [(4.5, 2.5, 0, 0)]

    aug = A2.SmallestMaxSize(max_size=10, p=1)
    result = aug(image=img, keypoints=keypoints)
    assert result["keypoints"] == [(9, 5, 0, 0)]
...
def test_resize_keypoints():
    ...
    aug = A2.Resize(height=100, width=5, p=1)
    result = aug(image=img, keypoints=keypoints)
    assert result["keypoints"] == [(4.5, 10, 0, 0)]

    aug = A2.Resize(height=50, width=10, p=1)
    result = aug(image=img, keypoints=keypoints)
    assert result["keypoints"] == [(9, 5, 0, 0)]
...
def test_multiplicative_noise_grayscale(image):
    m = 0.5
    aug = augmentoo.augmentations.noise.multiplicative_noise.MultiplicativeNoise(m, p=1)
    ...
    aug = augmentoo.augmentations.noise.multiplicative_noise.MultiplicativeNoise(elementwise=True, p=1)
...
def test_multiplicative_noise_rgb(image):
    ...
    aug = augmentoo.augmentations.noise.multiplicative_noise.MultiplicativeNoise(per_channel=True, p=1)
    params = aug.get_params_dependent_on_targets({"image": image})
    mul = params["multiplier"]
    assert mul.shape == (3,)
    result = aug.apply(image, mul)
    ...
    aug = augmentoo.augmentations.noise.multiplicative_noise.MultiplicativeNoise(elementwise=True, per_channel=True, p=1)
    params = aug.get_params_dependent_on_targets({"image": image})
    mul = params["multiplier"]
    assert mul.shape == image.shape
    result = aug.apply(image, mul)
...
def test_grid_dropout_mask(image):
    ...
    aug = A2.GridDropout(p=1, mask_fill_value=0)
    ...
    aug = A2.GridDropout(p=1, mask_fill_value=100)
    result = aug(image=image, mask=mask)
    assert result["image"].sum() < image.sum()
    assert result["mask"].sum() > mask.sum()
    ...
    mask = np.ones([256, 320], dtype=np.uint8)
    aug = A2.GridDropout(p=1, mask_fill_value=None)
    result = aug(image=image, mask=mask)
    assert result["image"].sum() < image.sum()
    assert result["mask"].sum() == mask.sum()
...
def test_gauss_noise_incorrect_var_limit_type():
    with pytest.raises(TypeError) as exc_info:
        A2.GaussNoise(var_limit={"low": 70, "high": 90})
...
def test_gaus_blur_limits(blur_limit, sigma, result_blur, result_sigma):
    ...
    aug = A2.Compose([augmentoo.augmentations.blur.gaussian.GaussianBlur(blur_limit=blur_limit, sigma_limit=sigma, p=1)])
...
def test_unsharp_mask_limits(blur_limit, sigma, result_blur, result_sigma):
    ...
    aug = A2.Compose([A2.UnsharpMask(blur_limit=blur_limit, sigma_limit=sigma, p=1)])
...
def test_unsharp_mask_float_uint8_diff_less_than_two(val_uint8):
    ...
    unsharpmask = A2.UnsharpMask(blur_limit=3, always_apply=True, p=1)
...
def test_shift_scale_separate_shift_x_shift_y(image, mask):
    aug = A2.ShiftScaleRotate(shift_limit=(0.3, 0.3), shift_limit_y=(0.4, 0.4), scale_limit=0, rotate_limit=0, p=1)
    data = aug(image=image, mask=mask)
    expected_image = FGeometric.shift_scale_rotate(
...
def test_glass_blur_float_uint8_diff_less_than_two(val_uint8):
    ...
    glassblur = augmentoo.augmentations.blur.glass_blur.GlassBlur(always_apply=True, max_delta=1)
...
def test_perspective_keep_size():
    ...
    transform_1 = A2.Compose(
        [A2.Perspective(keep_size=True, p=1)],
        keypoint_params=A2.KeypointParams("xys"),
        bbox_params=A2.BboxParams("pascal_voc", label_fields=["labels"]),
    )
    transform_2 = A2.Compose(
        [A2.Perspective(keep_size=False, p=1), A2.Resize(h, w)],
        keypoint_params=A2.KeypointParams("xys"),
        bbox_params=A2.BboxParams("pascal_voc", label_fields=["labels"]),
    )
...
def test_longest_max_size_list():
    ...
    aug = A2.LongestMaxSize(max_size=[5, 10], p=1)
    result = aug(image=img, keypoints=keypoints)
...
def test_smallest_max_size_list():
    ...
    aug = A2.SmallestMaxSize(max_size=[50, 100], p=1)
    result = aug(image=img, keypoints=keypoints)
...
@pytest.mark.parametrize(
    ...
    [
        (0.5, 0.5, augmentoo.augmentations.crops.random_sized_crop.RandomSizedCrop((50, 200), 513, 450, always_apply=True), (513, 450), (224, 224)),
        (0.3, 0.5, augmentoo.augmentations.crops.random_resized_crop.RandomResizedCrop(513, 450, always_apply=True), (513, 450), (224, 224)),
        (1.0, 0.5, augmentoo.augmentations.crops.center_crop.CenterCrop(500, 450, always_apply=True), (500, 450, 3), (512, 512, 3)),
        (0.5, 0.8, A2.Resize(513, 450, always_apply=True), (513, 450), (512, 512)),
        (0.5, 0.2, A2.NoOp(), (224, 224), (224, 224)),
        (0.5, 0.9, A2.NoOp(), (512, 512, 3), (512, 512, 3)),
        (0.5, 0.5, None, (512, 512), (512, 512)),
        (0.8, 0.7, None, (512, 512, 3), (512, 512, 3)),
        (
            0.5,
            0.5,
            A2.Compose([augmentoo.augmentations.blur.box_blur.Blur(), augmentoo.augmentations.crops.random_sized_crop.RandomSizedCrop((50, 200), 512, 512, always_apply=True), A2.HorizontalFlip()]),
            (512, 512),
            (512, 512),
        ),
    ],
...
def test_template_transform(image, img_weight, template_weight, template_transform, image_size, template_size):
    ...
    aug = A2.TemplateTransform(template, img_weight, template_weight, template_transform)
    result = aug(image=img)["image"]
...
def test_template_transform_incorrect_size(template):
    ...
    with pytest.raises(ValueError) as exc_info:
        transform = A2.TemplateTransform([template], always_apply=True)
...
def test_advanced_blur_float_uint8_diff_less_than_two(val_uint8):
    ...
    adv_blur = augmentoo.augmentations.blur.advanced_blur.AdvancedBlur(blur_limit=(3, 5), always_apply=True)
...
def test_advanced_blur_raises_on_incorrect_params(params):
    with pytest.raises(ValueError):
        augmentoo.augmentations.blur.advanced_blur.AdvancedBlur(**params)
