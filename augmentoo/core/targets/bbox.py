...
def denormalize_bbox(bbox, rows, cols):
    ...
    by image height. This is an inverse operation for :func:`~augmentoo.augmentations.bbox.normalize_bbox`.
...
class AxisAlignedBoxTarget(AbstractTarget):
    ...
    def validate_input(self, value: Union[np.ndarray, List]):
        ...
        if len(value.shape) != 2 or value.shape[1] != 4:
            raise RuntimeError("Input argument for axis-aligned bbox target must be an array of shape [N,4]")

        pass

    @classmethod
    def bbox_vflip(cls, bbox, rows, cols):
    ...
    @classmethod
    def bbox_hflip(cls, bbox, rows, cols):
